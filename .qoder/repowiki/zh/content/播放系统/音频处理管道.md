...
- [feature/playback/src/main/java/app/yukine/playback/manager/NativeAudioFocusController.kt](file://feature/playback/src/main/java/app/yukine/playback/manager/NativeAudioFocusController.kt)
- [app/src/main/java/app/yukine/playback/PlaybackServiceRuntime.java](file://app/src/main/java/app/yukine/playback/PlaybackServiceRuntime.java)
- [feature/playback/src/test/java/app/yukine/playback/NativeAudioFocusControllerTest.kt](file://feature/playback/src/test/java/app/yukine/playback/NativeAudioFocusControllerTest.kt)
</cite>

## 更新摘要
**所做更改**   
- 新增原生音频焦点控制器章节，详细说明 NativeAudioFocusController 的实现和两种工作模式
- 更新架构总览图，展示新的音频焦点管理流程
- 增强音频焦点管理部分，涵盖独占模式和合作模式的详细机制
- 添加音频焦点状态转换图和回调处理机制说明
- 更新性能考虑部分，包含焦点管理的优化策略
...
**最新更新**：引入 NativeAudioFocusController 替代 Media3 内部焦点管理，提供独占模式和合作模式，支持积极的焦点抢占和真正的同时音频输出能力。
...
NAFC["NativeAudioFocusController<br/>原生音频焦点控制器"]
end
subgraph "核心模型"
...
- **原生音频焦点控制器**：独立于 Media3 的音频焦点管理，支持独占模式和合作模式。
...
## 架构总览
整体流程从输入源到渲染输出，中间穿插格式转换、效果处理与分析分支。**新增原生音频焦点控制器管理音频焦点生命周期**。
...
participant Focus as "NativeAudioFocusController"
...
Focus->>Out : "管理音频焦点"
Eng->>Out : "提交至渲染"
Note over Focus,Out : 独占模式：积极抢占焦点<br/>合作模式：允许同时输出
```
...
### 原生音频焦点控制器（NativeAudioFocusController）
职责
- 完全接管音频焦点生命周期管理，独立于 Media3 的内部焦点处理。
- 支持两种工作模式：**独占模式**（EXCLUSIVE）和**合作模式**（COOPERATIVE）。
- 在独占模式下积极重新获取焦点，发送暂停广播停止竞争播放器。
- 在合作模式下不请求系统音频焦点，实现真正的同时输出。

关键特性
- **指数退避重试**：最多5次重试，间隔为120ms、240ms、480ms、960ms、1920ms。
- **防抖窗口**：80ms防抖防止焦点请求风暴。
- **音量保护窗口**：800ms内音量变化视为应用发起而非系统降音。
- **延迟焦点获取**：支持延迟焦点获取回调机制。

```mermaid
stateDiagram-v2
[*] --> IDLE : 初始化
IDLE --> HELD : acquire()成功
IDLE --> RECLAIMING : 焦点丢失+独占模式
HELD --> LOST : 焦点丢失
LOST --> RECLAIMING : 开始抢占
RECLAIMING --> HELD : 重新获取成功
RECLAIMING --> IDLE : 重试耗尽
HELD --> IDLE : release()
note right of RECLAIMING : 发送暂停广播<br/>指数退避重试
note right of LOST : 防抖窗口检查<br/>延迟抢占调度
```
...
### 播放服务运行时集成（PlaybackServiceRuntime）
职责
- 创建和管理 NativeAudioFocusController 实例。
- 根据 USB 独占设备或用户偏好自动选择焦点模式。
- 处理焦点状态变化并相应调整播放行为。

集成要点
- **永久禁用 Media3 内部焦点管理**：handleAudioFocus 设置为 false。
- **动态模式切换**：根据 usbExclusiveEnabled 或 audioExclusiveEnabled 决定模式。
- **智能焦点恢复**：播放状态变化时自动获取或释放焦点。

```mermaid
flowchart TD
Start(["播放服务启动"]) --> CreatePlayer["创建播放器"]
CreatePlayer --> DisableMedia3["禁用Media3焦点管理"]
DisableMedia3 --> CreateController["创建NativeAudioFocusController"]
CreateController --> SetMode{"确定焦点模式"}
SetMode --> |USB独占/用户偏好| Exclusive["设置独占模式"]
SetMode --> |默认| Cooperative["设置合作模式"]
Exclusive --> CheckPlaying{"是否正在播放?"}
Cooperative --> CheckPlaying
CheckPlaying --> |是| Acquire["立即获取焦点"]
CheckPlaying --> |否| Ready["等待播放开始"]
Acquire --> Ready
Ready --> End(["准备完成"])
```
...
- **原生音频焦点控制器独立于 Media3，直接管理系统音频焦点**。
...
PC --> AE["AudioEngine"]
PC --> NAFC["NativeAudioFocusController"]
AE --> BM["BufferManager"]
AE --> SA["SpectrumAnalyzer"]
AE --> BD["BassDetector"]
AE --> AEC["AudioEffectChain"]
AEC --> FX["Effects"]
NAFC --> AM["AudioManager"]
```
...
- **焦点冲突**：检查独占模式下的重试机制和防抖配置。
- **同时输出问题**：验证合作模式下是否正确禁用系统焦点请求。

定位方法
- 日志埋点：在关键节点记录耗时与缓冲水位。
- 指标采集：监控 CPU、内存、温度与电池消耗。
- 回放测试：录制处理前后 PCM 对比，定位问题环节。
- **焦点调试**：监控焦点状态变化和回调响应时间。
...
## 结论
Echo Android 音频处理管道以引擎为核心，结合效果链、分析器与缓冲区管理，实现了可扩展、低延迟、高性能的实时音频处理体系。**新增的原生音频焦点控制器提供了灵活的焦点管理策略，支持独占模式和真正的同时输出能力**。通过合理的采样率与缓冲策略、原生加速与自适应降级，可在不同设备上取得良好体验。扩展接口与插件化设计便于开发者快速实现自定义效果与高级分析功能。
...
### 原生音频焦点控制器使用指南
- 模式选择
  - 独占模式：适用于需要绝对音频控制权的应用场景。
  - 合作模式：适用于需要与其他应用同时输出的场景。
- 集成步骤
  - 在播放服务中创建控制器实例。
  - 根据业务需求设置合适的模式。
  - 实现回调接口处理焦点状态变化。
