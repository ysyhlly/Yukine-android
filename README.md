...
- 应用内流体云：完整 NowBar 或左右停靠胶囊均可上滑，沿当前位置直接形变到状态栏下方并居中显示歌词、播放进度和播放按钮；胶囊手势采用 24dp / 500dp/s 阈值，左右方向锁定放宽为横向位移达到纵向的 1.2 倍。普通顶部胶囊宽 188dp，点击主体可展开为更细长的 304 × 52dp 歌曲名与歌手详情，但不会放大到完整 NowBar。继续上滑或在其他页面向上滚动会收缩为顶部把手；把手贴合状态栏安全区下沿，只在下方保留 14dp 内容留白。下滑把手或在页面中向下滚动会重新展开流体云。顶部云向左下滑停靠左侧、向右下滑停靠右侧，接近直下则恢复进入顶部前的位置。该形态仅属于应用内 UI，不接入 ColorOS 系统流体云。暂停后的已解析流媒体会直接恢复当前播放器，不会重复拉取 URL。
...
- In-app fluid cloud: either the full NowBar or a docked capsule can be swiped directly into a centered capsule below the status bar while retaining lyrics, progress, and play/pause. Capsule gestures use 24dp / 500dp/s thresholds, and horizontal locking now requires only 1.2× the vertical drift. The normal top capsule is 188dp wide, and tapping its body opens a slimmer 304 × 52dp title-and-artist detail state without growing to full NowBar size. Another upward swipe or upward page scroll contracts it into a small handle aligned directly below the status-bar safe area, with 14dp of clearance only beneath it; pulling the handle down or scrolling the page downward expands the cloud again. A down-left drag docks left, a down-right drag docks right, and a nearly straight downward drag restores the side used before entering the top state. This is app-local UI and does not integrate with the ColorOS system fluid cloud. Paused resolved streams resume the loaded player directly without fetching another URL.
