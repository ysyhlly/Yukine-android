...
- QQ 音乐播放解析兼容：vkey 请求优先按 `mediaMid` 生成文件名，并仅在需要时回退旧的 `songMid + mediaMid` 拼接格式；有效的 `qqmusic_key` / `qm_keyst` 会参与 vkey 鉴权，`psrf_qqaccess_token` 仍随 Cookie 保留，Netscape `#HttpOnly_` Cookie 记录可导入。QQ CDN 返回的 HTTP SIP 会在本机转换为 HTTPS，避免 Android 的明文网络策略拦截播放；真实的会员、地区或登录失败原因不会再被通用“需登录”提示覆盖。
...
- QQ Music playback compatibility: vkey requests now prefer `mediaMid` filenames and fall back to the legacy `songMid + mediaMid` form only when needed. Valid `qqmusic_key` / `qm_keyst` values participate in vkey authentication, while `psrf_qqaccess_token` remains in the Cookie header; Netscape `#HttpOnly_` records are importable. HTTP QQ CDN SIP URLs are normalized to HTTPS locally so Android's cleartext policy does not block playback, and concrete membership, region, or login failures are no longer hidden behind a generic login prompt.
