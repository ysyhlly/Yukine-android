...
- 流媒体地址恢复：后台播放或冷启动恢复遇到临时 URL 过期时，会优先换用仍有效的缓存地址，并在播放失败后自动重新解析当前歌曲，不再要求重新进入网络页手动拉取 URL。
...
- Streaming URL recovery: background playback and cold-start queue restore reuse a still-valid cached source when available, then automatically re-resolve the current track after an expired temporary URL fails instead of requiring a manual URL refresh from the network page.
