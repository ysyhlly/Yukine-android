...
版本 1～19 先由 `YukineSchema.normalizeV20` 规范化为 Room v20 等价 schema，再依次执行 v21～v32 的幂等规范化；真实 v20～v31 数据库只执行其后续步骤，不能重新运行仍读取旧 `favorites.track_id` 的 v20 转换。新安装会额外恢复 `playlists.name TEXT NOT NULL UNIQUE` 的精确历史约束，避免 Room 注解表达能力差异改变既有语义。
...
- v1 至 v19 旧格式路由矩阵，以及 v15 至 v31 导出 Room schema 矩阵：每个受支持起始版本均可原子升级至 v32，升级后通过外键与完整性检查。
