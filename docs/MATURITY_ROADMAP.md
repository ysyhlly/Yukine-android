...
| 智能歌单 | 部分落地 | 流媒体每日推荐/心动推荐 + 本地只读虚拟集合（最近添加/最近播放/一周最爱/很久没听）已在曲库总览「为你推荐」架提供统一入口，经 `virtual:*` 只读虚拟组渲染，不写 playlists 表 | 列表级「加入队列」尚未接入；一周最爱排序暂用 28 天播放计数近似 |
...
缺口：
- 本地虚拟歌单已在曲库总览「为你推荐」架形成统一只读入口（最近添加/最近播放/一周最爱/很久没听），经 `virtual:*` 虚拟组渲染，不写 playlists 表；列表级加入队列仍待接入。
...
- 从曲库页或收藏页进入。
- 每个虚拟集合支持直接播放（列表头播放全部 + 行播放）与单轨收藏/下载；列表级加入队列待接入。
