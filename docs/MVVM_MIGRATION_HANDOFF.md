...
- Remaining risk and next candidates: `MainActivityBase` still has direct `playbackService.queueSnapshot()` call sites, so the next queue/read slice should inspect whether any call can move to an existing semantic queue/playback owner without adding a facade. The four remaining `playbackQueueStateOwner::currentTrack` dependencies still feed distinct runtime-state, position, notification-artwork, and Wi-Fi-lock owners; do not collapse them into a single Service supplier unless that also removes a real constructor/interface dependency.
