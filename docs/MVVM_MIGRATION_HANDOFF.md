...
- Verification for the batch passed with default Gradle daemon/workers via `.\gradlew.bat :app:testDebugUnitTest --tests app.yukine.MainActivityArchitectureContractTest --console=plain`; focused `git diff --check` and mojibake scans over `MainActivityBase.java` plus `MainActivityArchitectureContractTest.java` also passed. Device or emulator smoke remains out of scope because these slices do not change notification controls, lyrics runtime behavior, shutdown, service lifecycle, foreground behavior, or background playback.
