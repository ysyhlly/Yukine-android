...
| 后台播放 | 流媒体临时 URL 播放失败会由 `PlaybackStateEventControllerTest.resolvedStreamingPlaybackErrorRefreshesUrlAndSuppressesStaleError` 覆盖自动刷新入口；刷新结果通过 `replaceCurrentSourceAndResume` 热替换当前音源，保留队列、进度和继续播放意图。 | Home 后持续播放超过音源 URL 有效期，确认刷新期间不会停留在暂停态，再返回 UI 检查进度与通知同步。 |
