...
| 后台播放 | `PlaybackErrorRecoveryManagerTest.serviceUrlRefreshPreemptsRetryingTheExpiredUrl` 覆盖服务侧刷新优先于旧 URL 重试；`PlaybackStreamingUrlRecovery` 不依赖 Activity，强制绕过旧缓存解析新地址，再通过 `replaceCurrentSourceAndResume` 保留队列、进度和继续播放意图。 | Home/划掉任务后持续播放超过音源 URL 有效期，确认服务独立刷新且不会停留在暂停态，再返回 UI 检查进度与通知同步。 |
